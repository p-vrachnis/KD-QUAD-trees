...
class Tree2():
    ...
    def kNN(self, kNN, k,dst,l,node=0):
         ...
         if node.data:
           ...
           tempkNN=numpy.array(kNN)
           dst2 = (self.dist(tempdata, tempkNN ))
           if dst2 < tdst[len(tdst)-1]:
               if len(dst)>k-1:
                   dst[k-1]=dst2
                   l[k-1] = node.data
                   tdst[k] = dst2
                   for j in range(len(dst) - 1, 0, -1):
                    for i in range(j):
                       if dst[i] > dst[i + 1]:
                           temp = dst[i]
                           temp1 = l[i]
         ...
         if node.nw:
             n1 = numpy.array(node.nw.qdata)
             temp1 = (self.dist(n1, tempkNN))
             if (not node.sw and not node.se and not node.ne):
              #if temp1 < tdst[k]:
                 self.kNN(kNN, k, dst, l, node.nw)
             else:
                 if temp1 < tdst[len(tdst)-1]:
                     self.kNN(kNN, k, dst, l, node.nw)
              #self.kNN(kNN,k,dst, l,node.nw)
         if node.ne:
             n2 = numpy.array(node.ne.qdata)
             temp2 = (self.dist(n2, tempkNN))
             if (not node.sw and not node.se and not node.nw):
                 #if temp2 < tdst[k]:
                         self.kNN(kNN, k, dst, l, node.ne)
             else:
                 if temp2 < tdst[len(tdst)-1]:
                     self.kNN(kNN, k, dst, l, node.ne)
                #self.kNN(kNN, k, dst, l, node.ne)

         if node.sw:
             n3 = numpy.array(node.sw.qdata)
             temp3 = (self.dist(n3, tempkNN))
             if (not node.nw and not node.se and not node.ne):
                 #if temp3 < tdst[k]:
                         self.kNN(kNN, k, dst, l, node.sw)
             else:
                 if temp3 < tdst[len(tdst)-1]:
                     self.kNN(kNN, k, dst, l, node.sw)
                 #self.kNN(kNN, k, dst, l, node.sw)
         if node.se:
             n4 = numpy.array(node.se.qdata)
             temp4 = (self.dist(n4, tempkNN))
             if (not node.sw and not node.nw and not node.ne):
                 #if temp4 < tdst[k]:
                         self.kNN(kNN, k, dst, l, node.se)
             else:
                 if temp4 < tdst[len(tdst)-1]:
                     self.kNN(kNN, k, dst, l, node.se)
                 #self.kNN(kNN, k, dst, l, node.se)
    ...
    def kNN(self, kNN,k, dst,l,node=0,):
        ...
        if len (dst) < k:
         ...
         l.append(node.data)
            ...
            dst[k-1]=self.dist(tempdata, tempkNN)
            l[k-1]=(node.data)
        for j in range(len(dst) - 1, 0, -1):
            for i in range(j):
                if dst[i] > dst[i + 1]:
        ...
        if node.lchild:
            if node.rchild:
             nl = numpy.array(node.lchild.data)
             templ = (self.dist(nl, tempkNN))
             nr = numpy.array(node.rchild.data)
             tempr= (self.dist(nr, tempkNN))
             if (templ <= dst[len(dst)-1]):
                 self.kNN(kNN, k, dst, l, node.lchild)
                 #for i in range (k):
                 if tempr <= dst[len(dst)-1]:
...
def main_menu(tree):
    global el,data,vtree,dim,type,tdst,tempdst;
    if type == 1:
     try:
        print(colored("\n----------------------------\nMAIN MENU       K-D TREE\n----------------------------\n","grey"
                     ,attrs=["bold","dark" ]))
