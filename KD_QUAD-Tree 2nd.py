...
class Tree2():
            ...
            if len(newdata) > 1:
              ...
              # x = float(round(x, 0))
               ...
               for i in range(len(newdata)):
                    #print (newdata[i])
                    if newdata[i][0] >= newdata[median][0] and newdata[i][1] >= newdata[median][1]:
                        ne.append(newdata[i])
                    elif newdata[i][0] >= newdata[median][0] and newdata[i][1] <= newdata[median][1]:
                        se.append(newdata[i])
                    elif newdata[i][0] <= newdata[median][0] and newdata[i][1] >= newdata[median][1]:
                        nw.append(newdata[i])
                    elif newdata[i][0] <= newdata[median][0] and newdata[i][1] <= newdata[median][1]:
    ...
    def range_search(self, ranges, res, node=0):
       ...
       if node.nw or node.ne or node.se or node.sw:
        ...
        #elif node.qdata[0] > ranges[0][0] and node.qdata[1] > ranges[0][1] :
            if node.sw :
              res = self.range_search(ranges, res, node.sw)
        #elif node.qdata[0] > ranges[0][0] and node.qdata[1] < ranges[0][1]:
            if node.nw:
